        ...
        <>
                ...
                sneaker.sites_sizes_prices && 
                    sneaker.sites_sizes_prices.sites_sizes_prices &&
                        <Select onChange={siteChange} placeholder="Select Site">
                            {Object.keys(sneaker.sites_sizes_prices.sites_sizes_prices).map(key => (
            ...
            {selectedSite && 
                <Select placeholder="Select Size" onChange={sizeChange}>
                    {Object.values(sneaker.sites_sizes_prices.sites_sizes_prices[selectedSite]).map((val:any) => (
                        Object.keys(val).map((size) => (
                            <option>{`Size:${size}  ${new Intl.NumberFormat('en-US', {style: 'currency', currency:'USD'}).format(val[size]/100)}`}</option>
                        ))
                    ))}
                </Select>
            }
